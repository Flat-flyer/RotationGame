...
bool Scene0::OnCreate() {
	...
	//playerSprite->SetPosition(50.0, 20.0);
	return true;
}
...
void Scene0::Render() {
	Vec3 screenCoords = projection * playerSprite->GetPosition();
	SDL_Rect spritePosition;
	spritePosition.h = playerSprite->thePlayer->h;
	spritePosition.w = playerSprite->thePlayer->w;
	spritePosition.x = screenCoords.x;
	spritePosition.y = screenCoords.y;
	SDL_Surface* screenSurface = SDL_GetWindowSurface(window);														//player sprite surface
	SDL_FillRect(screenSurface, nullptr, SDL_MapRGB(screenSurface->format, 255.0f,255.0f,255.0f));		//changed color to white from blue.
	SDL_BlitSurface(playerSprite->thePlayer, NULL, screenSurface,&spritePosition);										//blitz it to the screen
